...
export class DEX extends ERC20 {
  ...
  public async withdraw(amount: number, nonce: number, v: string, r: string, s: string): Promise<any> {
    ...
    const result: any = await this.ethEngine.callFunction("withdraw", [this.contractAddress, convertedAmount, nonce.toString(), v, r, s], configParams, EthConfirmation.CONFIRMATION, abi, this._dexContractAddress);
    return result;
  }
